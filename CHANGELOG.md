...
- **0.16** (2026-03-22): Executed `P4-TSK-001` by defining the
  deterministic operator-interface contract in `PROTOCOLS.md` and
  `USAGE.md`, including menu states/transitions, required `PV` preview
  payload fields
  (`speed_kmh,flow_lpm,pump_duty,active_sections,distance_m,area_ha`),
  fixed preview cadence rules, and explicit reset-confirm handshake
  semantics.
...
- **0.19** (2026-03-22): Executed `P4-TSK-007` by replacing
  `DIAGRAMS.md` placeholders with concrete SEE/THINK/DO, operator
  state-flow, and wiring I/O dependency diagrams, and by closing
  operator-usage placeholders in `USAGE.md` with implemented reset/fault
  behavior notes.
