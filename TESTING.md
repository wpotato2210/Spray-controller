...
- Expected output markers (all required):
  ...
  - `== P6 flow calibration check ==`
  - `== P6 wheel calibration check ==`
...
- CI enforcement: `.github/workflows/validate.yml` runs `./scripts/validate.sh`
  on pull requests and pushes that modify gate-controlled inputs.
...
## Bench acceptance expectations

- Total-flow telemetry must remain bounded and deterministic under nominal,
  stale-input, and burst-input cases.
- HOLD must immediately produce the safe-output path and corresponding status
  telemetry on the next loop cadence.
- Section telemetry ordering must remain stable for all 3-section combinations.
- Preview cadence must stay fixed while distance and area counters advance only
  from wheel-derived motion and active width.
...
- Validate rate-control response using speed (km/h), width (m), and target
  rate (L/ha).
- Record accepted calibration constants in persistent storage, then reboot once
  to verify deterministic reload of the active profile.
