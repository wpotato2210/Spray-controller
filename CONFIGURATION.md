...
| `control` | `slew_limit_pwm_per_cycle` | 12.0 | Max PWM step per loop to
reduce command jumps. |
| `limits` | `min_speed_kmh` | 0.5 | Minimum speed for rate control (km/h). |
| `limits` | `max_flow_lpm` | 100.0 | Maximum allowed flow (L/min). |
| `limits` | `flow_fault_lpm_threshold` | 0.03 | Measured-flow threshold for
no-flow fault detection (L/min). |
| `limits` | `min_target_flow_for_fault_lpm` | 0.2 | Minimum target flow before
applying no-flow fault logic (L/min). |
